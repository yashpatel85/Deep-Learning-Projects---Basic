...
def YOLO(input_layer, NUM_CLASS, model='yolov4', is_tiny=False):
    if is_tiny:
        if model == 'yolov4':
            output_layers = YOLOv4_tiny(input_layer, NUM_CLASS)
        elif model == 'yolov3':
            output_layers = YOLOv3_tiny(input_layer, NUM_CLASS)
    else:
        if model == 'yolov4':
            output_layers = YOLOv4(input_layer, NUM_CLASS)
        elif model == 'yolov3':
            output_layers = YOLOv3(input_layer, NUM_CLASS)

    return tf.keras.Model(inputs=input_layer, outputs=output_layers)
...
def YOLOv3(input_layer, NUM_CLASS):
    ...
    return tf.keras.Model(inputs=input_layer, outputs=[conv_sbbox, conv_mbbox, conv_lbbox])
...
def YOLOv4_tiny(input_layer, NUM_CLASS):
    ...
    # Wrap outputs into a Keras model before returning
    return tf.keras.Model(inputs=input_layer, outputs=[conv_mbbox, conv_lbbox])
